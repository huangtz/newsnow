...
export class UserTable {
  ...
  async init() {
    await this.db.prepare(`
      CREATE TABLE IF NOT EXISTS newsnow.t_user (
        id TEXT PRIMARY KEY,
        email TEXT,
        data TEXT,
        type TEXT,
  ...
  async addUser(id: string, email: string, type: "github") {
    ...
    const now = Date.now()
    if (!u) {
      await this.db.prepare(`INSERT INTO newsnow.t_user (id, email, data, type, created, updated) VALUES (?, ?, ?, ?, ?, ?)`)
        .run(id, email, "", type, now, now)
      logger.success(`add user ${id}`)
    } else if (u.email !== email && u.type !== type) {
      await this.db.prepare(`UPDATE newsnow.t_user SET email = ?, updated = ? WHERE id = ?`).run(email, now, id)
  ...
  async getUser(id: string) {
    return (await this.db.prepare(`SELECT id, email, data, created, updated FROM newsnow.t_user WHERE id = ?`).get(id)) as UserInfo
  }

  async setData(key: string, value: string, updatedTime = Date.now()) {
    const state = await this.db.prepare(
      `UPDATE newsnow.t_user SET data = ?, updated = ? WHERE id = ?`,
  ...
  async getData(id: string) {
    const row: any = await this.db.prepare(`SELECT data, updated FROM newsnow.t_user WHERE id = ?`).get(id)
  ...
  async deleteUser(key: string) {
    const state = await this.db.prepare(`DELETE FROM newsnow.t_user WHERE id = ?`).run(key)
